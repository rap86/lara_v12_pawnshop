        ...
        <div class="modal-content border-0 shadow-lg">
            <div class="modal-header bg-danger text-white py-3">
                <h5 class="modal-title fw-bold" id="deleteUserModalLabel">
                    <i class="bi bi-exclamation-triangle me-2"></i>Delete System User
                </h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>

            <form class="deleteUserForm" action="" method="POST" data-confirm-delete>
                @csrf
                @method('DELETE')
                ...
                <div class="modal-body p-4 text-center">
                    ...
                    <p class="text-muted small">This action cannot be undone. You are about to permanently remove <strong id="delete_user_name" class="text-dark">this account</strong> from the system.</p>
                </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (deleteModalElement) {
        deleteModalElement.addEventListener('show.bs.modal', function (event) {
            ...
            const form = document.getElementsByClassName('deleteUserForm')[0];
