...
document.addEventListener('DOMContentLoaded', function () {
    ...
    confirmSubmitBtn.addEventListener('click', function () {
        ...
        // Start the countdown sequence (assigning to our top-scoped variable)
        countdown = setInterval(() => {
            ...
            if (timeLeft > 0) {
                ...
                // 5. Safe to submit the form to the database
                form.submit();
            }
        }, 1000); // Triggers every 1 second
    });

    // 5. Clean up when the modal is closed/dismissed
    confirmModalElement.addEventListener('hidden.bs.modal', function () {
        // Clear any running interval cleanly since countdown is now globally accessible here
        if (countdown) {
            clearInterval(countdown);
        }

        // Reset variable state back to defaults
        timeLeft = 3;

        // Restore button appearance entirely
        confirmSubmitBtn.disabled = false;
        confirmSubmitBtn.innerHTML = `Yes, Save Records`;
    });
});
</script>
